...
- **v1.2 增量 spec**: [`docs/superpowers/specs/2026-07-19-server-driven-cancelled-state-design.md`](docs/superpowers/specs/2026-07-19-server-driven-cancelled-state-design.md)
- **历史 spec(已废弃)**: `2026-06-28-dynamic-choice-box-rendering-design.md`(v0.3 软阻塞设计,仅作历史记录)

## v1.2 服务端 cancelled 事件 (2026-07-19+)

v1.0 起 `ask_user_choice` 工具在 `call()` 内 `await` 等待用户响应,当等待超时
(`asyncio.TimeoutError`)或被运行时取消(`asyncio.CancelledError`,例如 WebChat
"停止生成"、插件卸载)时,工具会沿用 v1.0 success 路径的同一 SSE 事件类型
`interactive_choice_resolved` 推一条 `reason: "cancelled"` 通知,WebChat 前端
据此将候选框翻成"已取消"非交互态(选项按钮消失,显示 muted header 与"已
...
- 工具超时 / 取消后前端候选框的"已结束"视觉反馈已通过 v1.2 的
  `interactive_choice_resolved {reason: "cancelled"}` 事件支持(参见上文
  "v1.2 服务端 cancelled 事件"小节)。
